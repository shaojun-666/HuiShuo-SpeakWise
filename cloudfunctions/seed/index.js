...
exports.main = async (event, context) => {
  ...
  if (action === 'create_collections') {
    // 创建集合（微信云开发集合是隐式创建的，这里测试连通性）
    return { code: 0, message: '无需手动创建，数据写入时自动创建集合' }
  }

  // 默认：导入数据
  // 先清空已有数据
  try {
    const countResult = await db.collection('scenarios').count()
    if (countResult.total > 0) {
      // 已有数据，检查是否需要覆盖
      if (event.force !== true) {
        return { code: 0, data: { skipped: true, message: `scenarios 集合已有 ${countResult.total} 条数据，如需重新导入请设置 force: true` } }
      }
      // 删除已有数据
      const list = await db.collection('scenarios').get()
      const tasks = list.data.map(item => db.collection('scenarios').doc(item._id).remove())
